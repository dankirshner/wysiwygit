  ...
  * @return mixed */
    ...
    public function __construct() {
        ...
        if (isset($_CONFIG['_sessionVar']) &&
            ...
            //error_log("_CONFIG['_sessionVar']: " . print_r($_CONFIG['_sessionVar'], true));
        ...
        //error_log("this->session: " . print_r($this->session, true));
        ...
        if ($this->config['debug']) {
            error_log("[uploader _construct] this->typeDir: " . $this->typeDir . ", this->typeURL: " . $this->typeURL);
        }
        ...
        if ($this->config['debug']) {
            error_log("[uploader _construct] upload folder");
        }
    ...
    public function upload() {
        ...
        if ($config['disabled'] || !$config['access']['files']['upload']) {
            ...
            $message = $this->label("You don't have permission to upload files.");
    ...
    protected function checkInputDir($dir, $inclType=true, $existing=true) {
        ...
        if ($this->config['debug']) {
            error_log("[checkInputDir] dir: $dir");
        }
        ...
        if ($inclType) {
            ...
            if ($this->config['debug']) {
                error_log("[checkInputDir] first: $first, type: " . $this->type);
            }
    ...
    protected function backMsg($message, array $data=null) {
        ...
        if ($this->config['debug']) {
            error_log("[backMsg] message: " . $message);
        }
