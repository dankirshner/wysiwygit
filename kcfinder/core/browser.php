...
class browser extends uploader {
    ...
    public function __construct() {
        ...
        if (isset($this->post['dir'])) {
            ...
            if ($this->config['debug']) {
                error_log("[browser _construct] dir: $dir");
            }
    ...
    public function action() {
        ...
        if ($this->config['debug']) {
            error_log("[browser->action] this->session['dir']: " . $this->session['dir']);
        }
    ...
    protected function act_browser() {
        if (isset($this->get['dir']) &&
            ...
            if ($this->config['debug']) {
                error_log("[browser->act_browser] this->session['dir']: " . $this->session['dir']);
            }
        }
    ...
    protected function act_init() {
        ...
        if ($this->config['debug']) {
           error_log("[act_init] return data: " . print_r($data, true));
        }
    ...
    protected function act_expand() {
        if ($this->config['debug']) {
            error_log("[act_expand] this->postDir(): " . $this->postDir());
        }
        $data = array('dirs' => $this->getDirs($this->postDir()));
        if ($this->config['debug']) {
            error_log("[act_expand] return data: " . print_r($data, true));
        }
        return json_encode($data);
    }
    ...
    protected function act_chDir() {
        ...
        if ($this->config['debug']) {
            error_log("[act_chDir] this->postDir(): " . $this->postDir());
        }

        $this->session['dir'] = $this->type . "/" . $this->post['dir'];
        if ($this->config['debug']) {
            error_log("[act_chDir] this->session['dir']: " . $this->session['dir']);
        }
    ...
    protected function getFiles($dir) {
        ...
        $files = dir::content($dir, array(
            'types' => "file",
            'excludedDirs' => $this->config['excludedDirs']   // DKMOD
        ));
        if ($files === false)
            return $return;
    ...
    protected function postDir($existent=true) {
        ...
        if ($existent && (!is_dir($dir) || !is_readable($dir))) {
            //error_log("[postDir] trace: " . print_r(debug_backtrace(), true));
            $this->errorMsg("Inexistant or inaccessible folder: $dir.");
        }
        return $dir;
    }
    ...
    protected function getDir($existent=true) {
        ...
        if (isset($this->get['dir']))
            $dir .= "/" . $this->get['dir'];
        if ($existent && (!is_dir($dir) || !is_readable($dir)))
            $this->errorMsg("Inexistent or inaccessible folder: $dir");
        return $dir;
    }
    ...
    protected function errorMsg($message, array $data=null) {
        if ($this->config['debug']) {
            error_log("[errorMsg] message: $message");
        }
