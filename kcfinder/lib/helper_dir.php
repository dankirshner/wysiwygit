    ...
    * @return mixed */
    ...
    static function content($dir, array $options=null) {
        ...
        while (($file = @readdir($dh)) !== false) {
            ...
            //DKMOD
            $type = @filetype("$dir/$file");
            //error_log("file: $dir/$file, filetype: $type");
            if ($type == 'unknown') {
               $type = 'dir';
            }

            //DKMOD - check for readlink().
            if ($options['followLinks'] && ($type === "link") && function_exists("readlink")) {
                $lfile = "$dir/$file";
                do {
                    $ldir = dirname($lfile);
            ...
            if ((($type === "dir") && (($file == ".") || ($file == ".."))) ||
               ...
               in_array($file, $options['excludedDirs'])   // DKMOD
            )
                continue;
